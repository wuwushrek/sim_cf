...
int main(int argc, char **argv)
{
  ...
  n.param("enable_logging_setpoint_pose", enable_logging_setpoint_pose, false);


  ROS_INFO("wait_for_service /add_crazyflie");
  ros::ServiceClient addCrazyflieService = n.serviceClient<crazyflie_gazebo::AddCrazyflie>("/add_crazyflie");
  addCrazyflieService.waitForExistence();
  ROS_INFO("found /add_crazyflie");
  crazyflie_gazebo::AddCrazyflie addCrazyflie;
  ...
  addCrazyflie.request.enable_logging_setpoint_pose = enable_logging_setpoint_pose;
  ...
  if (genericLogTopics.size() == genericLogTopicFrequencies.size())
  {
    ...
    {
      crazyflie_gazebo::LogBlock logBlock;
